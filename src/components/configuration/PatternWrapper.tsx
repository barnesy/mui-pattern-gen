...
import { ConfigurationModeContext } from '../../contexts/ConfigurationModeContext';
...
export const PatternWrapper: React.FC<PatternWrapperProps> = ({
  ...
  // Check if Configuration Mode context is available (not in iframe)
  const configurationContext = useContext(ConfigurationModeContext);
  const isInMainApp = !!configurationContext;
  const { registerPatternInstance, unregisterPatternInstance } = configurationContext || {};
  ...
  useEffect(() => {
    ...
    // Register with Configuration Mode context (only in main app)
  ...
  // This prevents external prop changes from overwriting Configuration Mode updates
