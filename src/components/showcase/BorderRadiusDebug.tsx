...
export const BorderRadiusDebug: React.FC = () => {
  ...
  const components = [
    { name: 'Button', element: <Button variant="contained" id="test-button">Button</Button> },
    { name: 'Paper', element: <Paper id="test-paper" sx={{ p: 2 }}>Paper</Paper> },
    { name: 'Card', element: <Card id="test-card"><CardContent>Card</CardContent></Card> },
    { name: 'TextField', element: <TextField id="test-textfield" label="TextField" variant="outlined" /> },
    { name: 'Chip', element: <Chip id="test-chip" label="Chip" /> },
    { name: 'Alert', element: <Alert id="test-alert" severity="info">Alert</Alert> },
    { name: 'Avatar', element: <Avatar id="test-avatar">A</Avatar> },
    { name: 'FAB', element: <Fab id="test-fab" size="small"><AddIcon /></Fab> },
  ];
    ...
    <Box>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 2, mb: 4 }}>
        {components.map((comp) => (
          <Box key={comp.name} sx={{ textAlign: 'center' }}>
            <Box sx={{ mb: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: 60 }}>
              {comp.element}
            </Box>
      ...
      <Table size="small">
        <TableBody>
          ...
          {components.map((comp) => {
            const expected = comp.name === 'Avatar' || comp.name === 'FAB' ? '50%' : `${theme.shape.borderRadius}px`;
            const actual = computedStyles[comp.name] || 'Loading...';
            const isCorrect = actual === expected || (expected === '4px' && actual.startsWith('4px'));
